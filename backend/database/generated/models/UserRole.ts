...
export interface UserRoleDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * const userRoleWithUserUUIDOnly = await prisma.userRole.findMany({ select: { userUUID: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many UserRoles and only return the `userUUID`
   * const userRoleWithUserUUIDOnly = await prisma.userRole.createManyAndReturn({
   ...
   * })
   * 
   * // Update zero or more UserRoles and only return the `userUUID`
   * const userRoleWithUserUUIDOnly = await prisma.userRole.updateManyAndReturn({
   *   select: { userUUID: true },
   *   where: {
   *     // ... provide filter here
   *   },
